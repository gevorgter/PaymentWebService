...
{
    public class AuthPipeline : PipelineData<AuthRequestDTO>
    {
        public int? _scheduledTransactionId = null;
        public CommonDTO.AuthTransaction _transaction;
        ...
        public override PipelineSteps[] GetPipelineSteps()
        {
            return new PipelineSteps[] {
                ...
                new PipelineSteps(SaveResultToDb, PipelineSteps.RUNCONDITIONS.ALL),
                new PipelineSteps(GenerateEventForHooks),
            };
        }
        public async Task GenerateEventForHooks()
        {
            var ev = new CommonDTO.AuthEventData()
            {
                transactionId = (int)_transaction.id,
                amount = _rq.amount
            };
            await EventRepository.SaveEvent(_accountId, (int)EVENTTYPE.TRANSACTION, (int)TRANSACTIONEVENTSUBTYPE.AUTH, ev.transactionId.ToString(), ev);
        }

        public async Task SaveTransactionToDb()
        {
            await TransactionRepository.SaveTransaction(_accountId, TransactionType.AUTHORIZE, -1, _rq.amount.amount, _transaction).ConfigureAwait(false);
        }
        ...
        {
            _transaction = Global._mapper.Map<AuthTransaction>(_rq);
